...
func newIndexCommand() *clix.Command {
	...
	cmd.Run = func(ctx *clix.Context) error {
		...
		return runIndex(ctx, ui, ctx.Args[0], outputDir, parseLanguageFilter(languageFilter), embeddings)
	}
...
func runIndex(ctx *clix.Context, ui *cliUI, rootPath, outputDir string, filters map[string]struct{}, embeddings bool) error {
	...
	for i, input := range inputs {
		...
		if err := engine.Index(ctx, input, codesearch.WithEmbeddings(embeddings)); err != nil {
