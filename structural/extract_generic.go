...
var genericPatterns = map[string][]genericPattern{
	"python": {
	...
	"typescript": {
		{regexp.MustCompile(`(?m)^\s*(?:export\s+)?class\s+([A-Za-z_][A-Za-z0-9_]*)\b`), SymbolKindClass},
		{regexp.MustCompile(`(?m)^\s*(?:export\s+)?interface\s+([A-Za-z_][A-Za-z0-9_]*)\b`), SymbolKindInterface},
		{regexp.MustCompile(`(?m)^\s*(?:export\s+)?enum\s+([A-Za-z_][A-Za-z0-9_]*)\b`), SymbolKindEnum},
		{regexp.MustCompile(`(?m)^\s*(?:export\s+)?(?:async\s+)?function\s+([A-Za-z_][A-Za-z0-9_]*)\b`), SymbolKindFunction},
		{regexp.MustCompile(`(?m)^\s*(?:export\s+)?const\s+([A-Za-z_][A-Za-z0-9_]*)\b`), SymbolKindConstant},
	},
	"java": {
...
// ExtractSymbolsGeneric extracts symbols from languages supported by regex-based
// structural patterns.
func ExtractSymbolsGeneric(filePath string, src []byte) ([]Symbol, error) {
	language := languageFromPath(filePath)
	patterns := genericPatterns[language]
	if len(patterns) == 0 {
		return nil, nil
	}
